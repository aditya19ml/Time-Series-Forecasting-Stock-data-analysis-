...
def LSTNetModel(CNNFilters, CNNKernel, initialiser,dropout, GRUUnits, window, skip, SkipGRUUnits,highway, input_shape):
	...
	# Apply a Conv2D that will transform it into data of dimensions (batchsize, time, 1, NumofFilters)
        C = Conv2D(filters= CNNFilters, kernel_size=(CNNKernel, m) )(C)
        C = Dropout(dropout)(C)
	...
	# Calculate the number of values to use which is equal to the window divided by how many time values to skip
        ...
        S    = PreSkipTrans(pt, int((window - CNNKernel + 1) / pt))(C)
        _, S = GRU(SkipGRUUnits, activation="relu", return_sequences = False, return_state = True)(S)
        S    = PostSkipTrans(int((window - CNNKernel + 1) / pt))([S,X])
